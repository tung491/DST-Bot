def wish_id_user(): 
    """ 
        Change the keys and the vaules == the id and the name of Facebook user who you want wish  
    """
...
def choose_wish():
    ...
    for id in input_data[0].keys():
        return_data.append((id,'Cảm ơn team Regiustea đã pha những cốc trà ngon cho mọi người \n Chúc ngủ ngon, {} ^^ <3'.format(input_data[0][id])))

    for id in input_data[1].keys():
        return_data.append((id, 'Chúc mọi người trong team PYMI ngủ ngon, {}. Con bot này được viết bằng Python ^^ <3'.format(input_data[1][id])))

    for id in input_data[2].keys():
        return_data.append((id, 'Chúc anh em trong 11A5 ngủ ngon, {} <3'.format(input_data[2][id])))
    
    for id in input_data[3].keys():
        return_data.append((id, 'Chúc anh em trong team 9c ngủ ngon, {} <3'.format(input_data[3][id])))
    
    for id in input_data[4].keys():
        return_data.append((id, 'Chúc các bro trong GHC ngủ ngon, {}'.format(input_data[4][id])))
